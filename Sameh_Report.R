...
current_trustee_subset <- active_trustee %>% filter(Fund %in% c("TF072236","TF072584", "TF072129","TF071630")) %>% select(temp.name) 
...
#%>%
 # filter(#`Child Fund Status` %in% input$risk_fund_status,
  #       `Trustee Fund Name` %in% current_trustee_subset$temp.name)
...
sum_df_non_GPURL$by_8_2020 <- df %>% filter(`Trustee Fund Name` %in% current_trustee_subset$temp.name) %>%
  select(`Uncommitted Balance`) %>% sum()
...
summary_risk_tab_2 <- function(temp.region="ALL"){
  ...
  NON.GPURL <- left_join(NON.GPURL,NON.GPURL.2)
    
  output <- left_join(GPURL,NON.GPURL,by="Disbursement Risk Level",suffix = c(" (GPURL)", " (Non-GPURL)"))
...
RISK.df.row <- 10
writeDataTable(wb,1, df, startRow = RISK.df.row, startCol = 1)
