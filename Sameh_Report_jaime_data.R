...
crews_data <- report_grants %>% filter(PMA=='no')

crews_data <- crews_data %>% rename("Window #" =`Window Number`,
                                          "Trustee #" = Trustee,
...
crews_data <- crews_data %>% mutate(`Window #` = as.numeric(`Window #`))
...
crews_data <- crews_data %>% select(unique(names(data))) %>% filter(!(`Child Fund #` %in% data$`Child Fund #`))
...
closed <- which(data$`Months to Closing Date`<1)

data$`Required Monthly Disbursement Rate =(Remaining/GrantAmount)/MonthClosing`[closed] <- "Closed"
